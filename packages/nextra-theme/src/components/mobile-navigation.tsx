...
export function MobileNavigation({
  ...
  function findActiveTitle(data: any, focusedRoute: string): string | null {
    for (const item of data) {
      if (item.route === focusedRoute) {
        return item.title;
      }

      if (item.children) {
        const activeTitle = findActiveTitle(item.children, focusedRoute);
        if (activeTitle) {
          return activeTitle;
        }
      }
                  ...
                  >
                    ...
                    {(!asPopover || !showSidebar) && (
                      <Collapse isOpen={showSidebar} horizontal>
                        <Menu
                          className="nextra-menu-desktop nx-gap-0.5"
